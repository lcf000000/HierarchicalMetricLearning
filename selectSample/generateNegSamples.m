...
%   Detailed explanation goes here
    load(['../feature/', dataset, '/', dataset, '_train_length.mat' ]);
    ...
    for i=1:class_num
        ...
        anchors = load(['../feature/', dataset, '/spdFeature_train/', file.name]);
        super_class = anchors.labels_1{1};
        sub_list = dir(['../feature/', dataset, '/spdFeature_train/', dataset, '_train_*_', num2str(super_class), '.mat']);
        subClass_num = length(sub_list);
        anchor_num = length(anchors.data_feature);
        for j=1:anchor_num
            anchor = anchors.data_feature(:, :, j);
            for p=1:subClass_num
                if ~strcmp(sub_list(p).name, file.name)
                    sample = load(['../feature/', dataset, '/spdFeature_train/', sub_list(p).name]);
                    selected_len = 0;
